...
{
	/* Optional userspace monitor is not implemented in this 4.19 patchset. */
}
...
{
	/* Legacy helper retained for KernelSU-Next compatibility. */
}
...
static __maybe_unused int susfs_add_try_umount_entry(struct st_susfs_try_umount* __user user_info) {
...
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
	enabled_features |= BIT_ULL(2);
#endif
#ifdef CONFIG_KSU_SUSFS_TRY_UMOUNT
	enabled_features |= BIT_ULL(3);
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME
	enabled_features |= BIT_ULL(4);
#endif
#ifdef CONFIG_KSU_SUSFS_ENABLE_LOG
	enabled_features |= BIT_ULL(5);
#endif
#ifdef CONFIG_KSU_SUSFS_HIDE_KSU_SUSFS_SYMBOLS
	enabled_features |= BIT_ULL(6);
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_CMDLINE_OR_BOOTCONFIG
	enabled_features |= BIT_ULL(7);
#endif
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
...
{
	static const char variant[] = SUSFS_VARIANT;
...
{
	static const char version[] = SUSFS_VERSION;
