...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* For newly created mounts, spoof mnt id only for KSU domain. */
					...
					int flag)
{
	...
	/*
	 * CL_COPY_MNT_NS identifies copy_tree() clone from copy_mnt_ns().
	 * For KSU domain:
	 * - single clone: allocate a fresh spoofed sus mnt_id
	 * - copy_mnt_ns clone: preserve original mnt_id for namespace copy
	 */
	...
	/* Zygote/other namespaces inheriting sus mounts get fresh spoofed ids. */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Reorder mnt_id for zygote normal mount clones only. */
